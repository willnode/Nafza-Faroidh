...
// RichEnum agar Enum <-> int bisa terkonversi secara implisit
        ...
        where TDerived : RichEnum<TValue, TDerived>
{
    ...
    private static void CheckInitialized()
    {
        if (!_isInitialized)
        {
            ...
            foreach (var field in fields)
            {
                ...
                instance._descriptionAttribute = null;// field.GetCustomAttributes<DescriptionAttribute>();
            }
            _isInitialized = true;
        }
    }
    ...
    public static TDerived Parse(string name)
    {
        foreach (TDerived value in _values.Values)
            if (0 == string.Compare(value.Name, name, true) || 0 == string.Compare(value.Name, name, true))
                return value;
